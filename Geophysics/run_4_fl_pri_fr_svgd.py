...
folderpath = '/home/ib012512/Documents/Results/re_m4_svgd/'+args.foldername
...
## setup variable
modelName = "model4flatprior_noDen"
...
std_upper = tfconstant(args.up_std*np.ones([num_per_layer]))
thickness_prior = 200* np.ones(num_per_layer)+np.random.normal(0,30,8)
std_lower = args.low_std
...
prior_mean = tf.concat([thickness_prior,upper_prior],axis = 0)
prior_std = tf.concat([std_lower*tf.ones(num_per_layer,tfdtype),std_upper],axis = 0)
num_para_total = num_para 
norm = gaussianNormalizer(prior_mean,prior_std)

# normalize prior mean and std
mean_prior_norm = norm.normalize(prior_mean)
upper_prior_norm = mean_prior_norm[num_per_layer:]
...
mu_true = tf.convert_to_tensor(np.array([200.        , 200.        , 200.        , 200.        ,
                                          ...
                                          850.02221393, 850.02221393, 706.28215182, 706.28215182]),tfdtype)
...
# learning_rate = args.Adamstepsize
# iterations = args.Adamstep
...
# print('stat_model.joint_log_post(True):',stat_model.joint_log_post(norm.normalize(mu_true),monitor = True))
...
# num_results = args.mcmcstep
# number_burnin = 0

# # RMH
# step_size = args.rmh_stepsize
...
# # print(f'acceptance rate Hamiltonian Monte Carlo {num_results:d} {accept_HMC_10:.2f}%')
...
# # foldername = 're_m4_hmc001_1050_std001'
...
#                         'MAP':MAP.numpy(),
#                         }, cls=NumpyEncoder)
# with open(folderpath+'/MCMC_'+modelName+'.json', 'w') as outfile:
#     json.dump(json_dump, outfile)
...
mu,mu_list = stat_model.svgd(svgd_args.num_particles,svgd_args.num_iter,svgd_args.lr)

json_dump = json.dumps({'mu':mu.numpy(),
                        'mu_list':np.array(mu_list)
                        }, cls=NumpyEncoder)
with open(folderpath+'/SVGD_'+modelName+'.json', 'w') as outfile:
    json.dump(json_dump, outfile)
