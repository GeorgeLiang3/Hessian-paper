...
class Stat(object):
    ...
    def svgd(self,num_particles = 300,num_iter = 200,lr = 5):

        self.sv = SVGD(self.graph_joint_log_post,num_particles=num_particles,num_iter = num_iter, dtype=self.tfdtype)
        self.sv.lr = lr

        # generate initial particals from Prior distribution
        mvn_prior = tfd.MultivariateNormalDiag(
                    loc=tf.zeros(self.Number_para_total,tf.float64),
                    scale_diag = 0.5*tf.ones(self.Number_para_total,tf.float64))
        # generate initial particals from Prior distribution
        mvn_prior_ = tfd.Sample(mvn_prior,sample_shape=([num_particles]))
        sample = mvn_prior_.sample()
        mu_init = sample
        start = timeit.default_timer()
        mu,mu_list =self.sv.run_chain_svgd(mu_init)
        end = timeit.default_timer()
